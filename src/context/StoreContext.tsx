...
import { Product, Category, CartItem, UserAuth, Order, Inquiry, BlogPost, ViewPage } from '../types';
...
interface StoreContextType {
  ...
  // Product Catalog CRUD & Order Sequence
  products: Product[];
  ...
  // Shopping Cart & Instant Purchase
  cart: CartItem[];
  isCartOpen: boolean;
  ...
  toggleAllCartItemsCheck: (isChecked: boolean) => void;
  ...
  // Orders Management & Direct Checkout
  orders: Order[];
  ...
  inquiries: Inquiry[];
  addInquiry: (inquiryData: Omit<Inquiry, 'id' | 'date' | 'status'>) => void;
...
export const StoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [auth, setAuth] = useState<UserAuth>(() => {
    const saved = localStorage.getItem('techrcpro_auth');
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('techrcpro_auth', JSON.stringify(auth));
  }, [auth]);

  const login = (email: string, role: 'customer' | 'admin' = 'customer') => {
    const name = email.split('@')[0] || 'Valued Racer';
  ...
  const continueDemoMode = (role: 'customer' | 'admin' = 'customer') => {
    setAuth({
      ...
      isDemoMode: true,
      user: {
        name: role === 'admin' ? 'Demo Admin' : 'Demo Racer',
        email: role === 'admin' ? 'admin@techrcpro.com' : 'racer@techrcpro.com',
        role
      }
    });
    setCurrentPage(role === 'admin' ? 'admin' : 'home');
    showToast(`Entered Demo Mode as ${role === 'admin' ? 'Admin' : 'Racer'}`);
  };
  ...
  const [products, setProducts] = useState<Product[]>(() => {
    const saved = localStorage.getItem('techrcpro_products');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { /* ignore */ }
    }
  ...
  useEffect(() => {
    localStorage.setItem('techrcpro_products', JSON.stringify(products));
  }, [products]);
  ...
  const resetProductsToDefault = () => {
    ...
    localStorage.removeItem('techrcpro_products');
    showToast('Reset catalog to default TechRcPro collection');
  };

  // --- Categories State ---
  const [categories, setCategories] = useState<Category[]>(() => {
    const saved = localStorage.getItem('techrcpro_categories');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { /* ignore */ }
    }
  ...
  useEffect(() => {
    localStorage.setItem('techrcpro_categories', JSON.stringify(categories));
  }, [categories]);
  ...
  const [cart, setCart] = useState<CartItem[]>(() => {
    const saved = localStorage.getItem('techrcpro_cart');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { /* ignore */ }
    }
    return [
      { product: INITIAL_PRODUCTS[0], quantity: 1, isChecked: true },
      { product: INITIAL_PRODUCTS[1], quantity: 1, isChecked: true }
    ];
  });
  ...
  useEffect(() => {
    localStorage.setItem('techrcpro_cart', JSON.stringify(cart));
  }, [cart]);
      ...
      if (existing) {
        return prev.map(item =>
          item.product.id === product.id
            ? { ...item, quantity: item.quantity + quantity, isChecked: true }
            : item
        );
      }
      return [{ product, quantity, isChecked: true }, ...prev];
    });
  ...
  const toggleAllCartItemsCheck = (isChecked: boolean) => {
    setCart(prev => prev.map(item => ({ ...item, isChecked })));
  };
  ...
  const [orders, setOrders] = useState<Order[]>(() => {
    const saved = localStorage.getItem('techrcpro_orders');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { /* ignore */ }
    }
  ...
  useEffect(() => {
    localStorage.setItem('techrcpro_orders', JSON.stringify(orders));
  }, [orders]);
  ...
  const exportOrdersToCSV = () => {
    ...
    const baseUrl = typeof window !== 'undefined' ? window.location.origin : 'https://techrcpro.com';
    ...
    link.href = url;
    link.setAttribute('download', `techrcpro_orders_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const [inquiries, setInquiries] = useState<Inquiry[]>(() => {
    const saved = localStorage.getItem('techrcpro_inquiries');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { /* ignore */ }
    }
  ...
  useEffect(() => {
    localStorage.setItem('techrcpro_inquiries', JSON.stringify(inquiries));
  }, [inquiries]);
  ...
  const addInquiry = (inquiryData: Omit<Inquiry, 'id' | 'date' | 'status'>) => {
    ...
    showToast('Technical inquiry submitted! Our pit engineers will respond shortly.');
  };
  ...
  const exportInquiriesToCSV = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.setAttribute('download', `techrcpro_inquiries_${new Date().toISOString().slice(0,10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const [blogs, setBlogs] = useState<BlogPost[]>(() => {
    const saved = localStorage.getItem('techrcpro_blogs');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { /* ignore */ }
    }
  ...
  useEffect(() => {
    localStorage.setItem('techrcpro_blogs', JSON.stringify(blogs));
  }, [blogs]);
  ...
  const addBlog = (blogData: Omit<BlogPost, 'id' | 'date'>) => {
    ...
    showToast(`Tuning guide "${newBlog.title}" published!`);
  };
  ...
  const updateBlog = (updatedBlog: BlogPost) => {
    ...
    setBlogs(prev => prev.map(b => b.id === updatedBlog.id ? { ...updatedBlog, slug: formattedSlug } : b));
    showToast(`Guide "${updatedBlog.title}" updated`);
  };
