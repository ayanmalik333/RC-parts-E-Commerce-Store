...
export const AdminPanelPage: React.FC = () => {
  ...
  const [prodCategory, setProdCategory] = useState(categories[0]?.name || 'Brushless Motors & ESCs');
  const [prodPrice, setProdPrice] = useState('119');
  const [prodOrigPrice, setProdOrigPrice] = useState('139');
  ...
  const [blogCategory, setBlogCategory] = useState('Powertrain & ESCs');
  const [blogAuthor, setBlogAuthor] = useState('Marcus Vance, Chief Dyno Technician');
  ...
  const openNewProductModal = () => {
    ...
    setProdName('');
    setProdCategory(categories[0]?.name || 'Brushless Motors & ESCs');
    setProdPrice('119');
    setProdOrigPrice('139');
    setProdDesc('High-torque competition grade RC component.');
    setProdImage('https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&w=800&q=80');
  ...
  const handleSaveProduct = (e: React.FormEvent) => {
    ...
    if (editingProduct) {
      ...
      addProduct({
        ...
        description: prodDesc,
        specs: {
          'Scale': '1/10 & 1/8 Competition',
          'Voltage': '2S - 6S LiPo Support',
          'Connector': '4.0mm Gold Bullet / Amass XT60'
        },
        rating: 5.0,
        reviewCount: 1,
        images: [prodImage || 'https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&w=800&q=80'],
        isFeatured: prodIsFeatured,
        sequenceOrder: parsedSeq,
        stock: parsedStock,
        inStock: parsedStock > 0
  ...
  const openNewBlogModal = () => {
    setEditingBlog(null);
    setBlogTitle('');
    setBlogSlug('/blogs/my-new-tuning-guide');
    setBlogMetaTitle('My New Tuning Guide | TechRcPro');
    setBlogMetaDescription('Discover our latest RC tuning guide on brushless motor KV ratios and ESC timing profiles.');
    setBlogContent('<h1>My New Tuning Guide</h1>\n<p>Write your RC guide here or upload a raw .html file above.</p>');
    setBlogCategory('Powertrain & ESCs');
    setBlogAuthor('Marcus Vance, Chief Dyno Technician');
    setBlogImage('https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&w=1000&q=80');
    setBlogExcerpt('Discover our latest RC tuning guide on brushless motor KV ratios and ESC timing profiles.');
  ...
  const openEditBlogModal = (b: BlogPost) => {
    ...
    setBlogMetaTitle(b.metaTitle || `${b.title} | TechRcPro`);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-bold uppercase tracking-widest text-[#38BDF8]">
                TechRcPro Operations Hub
              </span>
              <h1 className="font-serif text-2xl sm:text-4xl font-bold">
                TechRcPro Store Operations Dashboard
              </h1>
            </div>
          </div>
            ...
            <form onSubmit={handleSaveBlog} className="space-y-4 text-xs">
              ...
              <div>
                ...
                <input
                  ...
                  type="text"
                  required
                  placeholder="e.g. Brushless Motor KV vs Gear Ratio Tuning Guide"
                  value={blogTitle}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-[#F7F4EE] border border-[#E6E0D8] rounded-xl focus:outline-none"
                  >
                    <option value="Powertrain & ESCs">Powertrain & ESCs</option>
                    <option value="LiPo Battery Care">LiPo Battery Care</option>
                    <option value="Chassis & Suspension">Chassis & Suspension</option>
                    <option value="Transmitters & Telemetry">Transmitters & Telemetry</option>
                  </select>
                </div>
              </div>
              ...
              <div className="bg-[#F7F4EE] p-4 rounded-2xl border border-[#E6E0D8] space-y-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    type="text"
                    placeholder="e.g. Brushless Motor & ESC Gearing Guide | TechRcPro"
                    value={blogMetaTitle}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    placeholder="e.g. Master your brushless motor KV calculations and ESC timing profiles for maximum track speed and thermal endurance."
